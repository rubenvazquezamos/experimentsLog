...
**Conclusion:** this doesn't seem to be the case! See the experiment `paramRepsQRD5` in the section about the effect of [panel repetitions](#Panel-repetitions) on the diffusion coefficient. It shows that the phase agreement improves when periodicity is present, but the diffusion coefficient agreement does not.
